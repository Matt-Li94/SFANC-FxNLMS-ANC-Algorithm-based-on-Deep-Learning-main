...
# real noises
fs = 16000
StepSize = 0.0001 #原来是0.0001 学习率0.002暂时不要改变
...
controller = VSS_FxNLMS_mu(Len=1024,mu_min=min, mu_max=max, beta=beta) # 1024 is the same size of coeffient vector of fixed-filter
ErrorVSSFxNLMS = train_vss_fxnlms_algorithm_mu(Model=controller, Ref=Fx, Disturbance=Dis, mu_min=min, mu_max=max, beta=beta ,as0=as0, as1=as1) #训练VSS_FxNLMS模型

sf.write('./Real Noise Examples/vssfxlms1.wav', ErrorVSSFxNLMS, fs, format='wav')
...
# 绘制第一个子图：ErrorFxNLMS
plt.subplot(4, 1, 1)
plt.plot(time_sequence, nc, color='green', label='noise-music')
# 设置 y 轴范围
plt.ylim(-1, 1)
plt.ylabel('Magnitude')
plt.xlabel('Time (seconds)')
...
# 绘制第二个子图：ErrorVSSFxNLMS
plt.subplot(4, 1, 2)
plt.plot(time_sequence, ErrorFxNLMS, color='red', label='ErrorFxLMS')
# plt.ylim(-0.35, 0.35)
plt.ylabel('Magnitude')
plt.xlabel('Time (seconds)')
plt.legend()
...
# 绘制第三个子图：Dis
plt.subplot(4, 1, 3)
plt.plot(time_sequence, ErrorVSSFxNLMS, color='blue', label='ErrorVSSFxLMS')
# plt.ylim(-0.35, 0.35)
plt.ylabel('Magnitude')
plt.xlabel('Time (seconds)')
plt.legend()
...
# 绘制第三个子图：Dis
plt.subplot(4, 1, 4)
plt.plot(time_sequence, waveform11, color='yellowgreen', label='music')
# plt.ylim(-0.35, 0.35)
plt.ylabel('Magnitude')
plt.xlabel('Time (seconds)')
plt.legend()
...
# # 绘制sum_error_sfanc_fxnlms_np1的波形图
# plt.figure()
# plt.plot(time_sequence11, sum_error_sfanc_fxnlms_np1, color='blue')
...
# adjusted_gain_values = adaptive_control_gain_algorithm(Error_SFANC_FxNLMS1, Fx1, max_gain=1.5, min_gain=0.5,
#                                                        step_size=0.1)
...
deg_file = './Real Noise Examples/vssfxlms1.wav'  # 滤波音频 评估文件
